...
#
# Tune these if you like (tokens currently in the context window):
RECOMMEND_AT=${CREDIT_SAVER_RECOMMEND_TOKENS:-200000}   # tokens in context where a handoff starts saving real money

input=$(cat)
command -v jq >/dev/null 2>&1 || exit 0

# Muted? The user ran /handoff-hide — stay completely silent until /handoff-show.
...
# directive that forbids running any tool/command/skill — the model only echoes
# the final line.
emit() {
  jq -n --arg line "$1" '
